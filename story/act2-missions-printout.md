# Temple of the QRians — Act 2 Mission Reference

**15 missions across 3 houses. ~20-minute act under sealing-temple time pressure (stone doors closing, passages narrowing). Expected completion: TBD.**

**Setting**: The Green Department — ancient botanical/medicinal gardens above the Act 1 antechamber. Reached by Drake's Construction Hoist (Act 1 success), Croft's Grappling Rigs (Act 1 success), or a flood-failsafe.

**Source-evidence types delivered across Act 2**:
- **S0** — The Source itself (geographic radiation field, place-bound, indestructible)
- **S1** — Language drift / The Gift (alphabet was concepts before letters; blocky glyphs = Source's imprint)
- **S2** — The Obsession (Priest Scientists, Prophet Mathematicians — religion + math fused; compulsive classification)
- **S3** — The Contagion ("Chaotic Order" / "logical contagion" — they had a word for it and couldn't stop it)
- **S4** — The Seal (temple built by sedated/eliminated slave-builders; civilization extincted itself voluntarily)
- **S5** — Self-influence (players themselves are starting to feel the Source's pull)

**Trio synthesis** (across all three R-typed missions): the QRian script the team has been decoding all year *is* the Source's transmission medium. To write QRian was to be touched. To read it... is what they have been doing.

**Puzzle type coverage**: 5 trios × 3 houses = 15 slots. Each house uses J/M/R/C/N. Types H and D do not appear in Act 2.

**Status legend**:
- ✅ FULLY DESIGNED — all text locked
- 🚧 IN PROGRESS — briefs/consequences locked; Anthony to fill 5 concepts each (C-trio only)

---

# DRAKE DELEGATION (Red)

## A2M1 — Powder of the Quiet Bed (Artifact)  ✅

**Type:** J — Hidden Symbol Hunt (emergent composition variant) | **Cards:** 3 ("Apothecary Note") | **Evidence:** S4 — The Seal | **Glyph teaching:** B introduced

**Story card brief:**
> Sweeping across the far wall bedded in vines, you stop. What looked like a garden — dozens of flower and leaf arrangements pinned to the vine surface — is actually a grid. The same few shapes, repeating in patterns. Your medic says it first: it's writing. The whole wall is writing. Scattered across the chamber floor are loose working notes — the pharmacist's own records. You'll want to gather those before the wall means anything…

**Puzzle:** Players receive 1 printed grid (the flower-wall as QRian glyphs). Each card identifies one batch code (A, B, C). For each batch, mark all instances of that glyph on the grid bearing harvest-sequence numbers. Connect them in numerical order — the connecting line traces a QRian super-glyph. Three super-glyphs spell **B-Y-E**.

**Clue cards:**
- **Card 1 — Folded Apothecary Note**: "Batch A — Soporific base compound. Mark all Batch A specimens. Connect in harvest order." [QRian glyph A]
- **Card 2 — Ink-stained Apothecary Note**: "Batch B — Activating agent. Mark all Batch B specimens. Connect in harvest order." [QRian glyph B — **NEW**]
- **Card 3 — Torn Apothecary Note**: "Batch C — Terminal compound. Mark all Batch C specimens. Connect in harvest order." [QRian glyph C]

**Answer:** `bye` (alternatives: `BYE`, `Bye`, `goodbye`, `farewell`)

**Hint:** *The pharmacist categorized three batches — A, B, C — across the wall. Mark each one separately. The pattern of each batch, connected in sequence, is what the pharmacist really wanted you to read.*

**Correct answer reveal:**
"The three shapes resolve: B — Y — E. Not a formula. Not a dosing instruction. A farewell. The pharmacist embedded a goodbye in the arrangement of every plant in this room. Batch A was the sedative. Batch B was the paralytic. Batch C was the terminal compound. Together they didn't just quiet the builders — they ended them. The QRians said goodbye to their own labor force in the language of botany, and left the word hidden in the wall they built."

**Success consequence:**
"The wall said BYE. The pharmacist who catalogued every plant in this room knew what the compound did and knew who would drink it. Logged. The Green Department wasn't a medicine garden. It was an execution chamber dressed up as horticulture. Move."

**Failure consequence:**
"The grid means nothing without the notes to decode it. We're carrying a map we can't read. Whatever the pharmacist was tracking in those batch ledgers stays in the wall. File the grid as unresolved intelligence and press on."

---

## A2M2 — Drevu's Compartment (Environment)  ✅

*Replaces Cascade Triage. Drake's N-typed mission, separate alcove, table-passcode mechanism.*

**Type:** N — Conceptual Procedure | **Cards:** 6 tile-inserts | **Evidence:** S4 expansion (slave-labor temple construction)

**Story card brief:**
> Off the side-passage and into a dim alcove: a stone table set in the corner, surface scored with rows of inset tile-slots — some filled, some empty. Above the table, a sealed compartment, no hinge. Your bomb-handler reads the gaps. *Passcode lock. The other houses are holding our missing tiles.*

**Puzzle:** Stone table holds 2 procedure-rows of 4 inset tile-slots. Slots 1-3 carry word-tiles; slot 4 is the passcode-derivation blank. Tiles distributed 1+1+1 across houses. Solve both procedures, enter combined two-word passcode, compartment opens.

**Procedure 1:** drop → crack → split → ?? *(fragile object failing in stages)*
**Accepted:** shatter, fragment, break, splinter, burst, crumble

**Procedure 2:** date → move-in → marry → ?? *(romantic timeline — DELIBERATELY PESSIMIST)*
**Accepted:** divorce, separate, breakup, end, regret, die *(PESSIMIST ONLY — optimistic answers like "have kids", "honeymoon" REJECT to fire the hint)*

**Hint** (after first failure on procedure 2): *"The QRians were a deeply pessimistic people. They were known to record the disappointing or tragic ending of any process they observed."*

**Correct answer reveal:**
"Both passcodes enter. The compartment opens. Inside: a folded record by **Mason Lamenter Drevu**.
> *'We had no choice. The Source corrupts those who linger. We cannot send our scholars to build, our priests, our mathematicians — any of them taken by the work, taken by the place, lost to the obsession. So we sent the others. The unprotected. The ones who would carry the work but not the worth. They build without knowing what they build. They will not return. None of them. We have written the order. We will see it through. Forgive us. There was no other way.'*"

**Success consequence:**
"Logged. Drevu wrote it down. He knew. He couldn't stop it. He asked forgiveness anyway. The QRians knew what they were doing to the builders, and at least one of them never made peace with it."

**Failure consequence:**
"The compartment stays sealed; the lock holds. The host's voice goes quiet — *we came close.* Drevu's record stays inside. We will not learn whose forgiveness he asked for, or whether he meant it. File and press on."

---

## A2M3 — The Wall of Repetitions, Red (Utility)  ✅

*Was Siegebloom Ledger. Now part of the M-trio (co-located with Jones-Amber + Croft-Purple in the Wall of Repetitions chamber).*

**Type:** M — Corrupted Reconstruction | **Cards:** 3 red tiles | **Evidence:** S2 + S5 (joint, all three houses combined)

**Story card brief:**
> Through the doorway and into the long chamber: the temple's east wall stretches floor-to-ceiling, carved with the same QRian phrase repeated over and over — hundreds of stacked rewrites, each eroded in its own way. Behind you, you realize: Jones and Croft are already at their own sections of the wall, working amber tiles and purple. Your bomb-handler crouches at the red tiles. *They wrote it this many times so the eroded ones could still be read against the rest.* Some of yours are with the others. You'll want them back to read the wall whole…

**Puzzle:** 3 tiles of each color, distributed 1+1+1. Each tile shows the same phrase with ~⅓ of letter positions worn smooth (`_`). Blanks are partitioned: each letter-position blanked on exactly one tile. Pool 3 same-color tiles, fill blanks from the other two tiles, reconstruct phrase.

**Drake's red tiles** (phrase: "IT WAS MAKING US OBSESSED"):
- Red 1 (held by Drake): `_T W_S M_KI_G U_ OB_ES_ED`
- Red 2 (held by Jones): `I_ WA_ MA_IN_ US _BS_SS_D`
- Red 3 (held by Croft): `IT _AS _AK_NG _S O_SE_SE_`

**Each tile's footer:** *"One of three eroded transcriptions of the same QRian phrase. The wall was carved many times so corruption could be averaged out across copies. Combine with the other two to read it whole."*

**Answer:** `it was making us obsessed`

**Hint:** *The wall was carved many times by different hands across centuries. Each tile shows a different set of worn-smooth letters — the gaps don't overlap. Lay all three of your color's tiles side by side. At every blank, look at the other two tiles: the letter is there.*

**Correct answer reveal:**
"The wall says: **IT WAS MAKING US OBSESSED**. Not the Source. The PLACE. The QRians felt it happening — they were the ones writing it down, watching themselves go obsessive, and writing it down was part of what was making them obsessive. Jones found a line above this one. Croft found another. Read all three together when the host calls it: the QRians wrote you a sentence across the whole wall."

**Joint host reveal** (after all three houses solve):
"Three houses. Three lines. One confession.
> *THIS PLACE ONCE MADE US WISE*
> *UNTIL WE SLOWLY REALIZED*
> *IT WAS MAKING US OBSESSED*

The QRians wrote it on the wall hundreds of times, in the only way that would survive the wall failing. They wanted you to read it. They wanted *anyone* to read it. They diagnosed themselves in the act of being broken, and they wrote the diagnosis at scale."

**Success consequence:**
"Logged. They knew. They weren't taken by surprise — they watched it happen to themselves, in themselves, and wrote it down. They wrote it down many times so the wall would still say it after the wall failed. We're reading the confession of a culture that diagnosed itself in the act of being broken."

**Failure consequence:**
"Without the red tiles, the closing line is unreadable. We caught only fragments — *MAKING US* — and don't know what made them what. File the wall as conclusive of nothing and press on."

---

## A2M4 — The Reagent Alcove (Path)  🚧

*NEW mission filling Drake's missing C-type. Briefs/consequences locked. **5 reagent-combinations TBD by Anthony**.*

**Type:** C — Conceptual Decipherment | **Cards:** 3 (bark labels) | **Evidence:** S4 expansion (the Seal as voluntary moral choice over scorched-earth)

**Story card brief:**
> One of your crew leans against the wrong stretch of wall and a panel swings inward. Behind: a low-ceilinged alcove, every surface stacked with sealed glass phials, ceramic crucibles, fired-clay jars. The smell stops you. *Chemicals.* Not medicine — engineering. Your bomb-handler crouches. *They were making something here.* Bark labels lie scattered across the floor, torn loose when the panel slammed. The other houses have a few. You'll want them back…

**Puzzle:** Each station-cluster of 2-3 reagents combined into one named compound. Match scattered card-labels back to stations, identify the 5 derived compounds. **Anthony's intent: area-weapon / geographic kill-zone compounds.**

**Puzzle prompt:** *"Name each compound the QRians were producing."*

**5 compounds:** TBD by Anthony.

**Hint:** *Each station-cluster of 2-3 reagents was making one named compound. Match the loose bark-labels back to their stations. Read each cluster as a recipe. Five compounds together tell you what they were trying to build — and why they stopped.*

**Correct answer reveal:**
"Five compounds resolve. Read together, they aren't medicine. They aren't preservatives. They aren't anything benign. Each compound, once produced, would render an area uninhabitable — soil poisoned, air corrupted, water altered, geography itself made hostile. The QRians were building *area-weapons*. Geographic kill-zones. They were planning to render their own region unfit for life. And then — partway through the program — they stopped. The half-finished compounds in the back of this alcove are the proof. They had the chemistry. They could have done it. They chose not to. They chose this temple instead. The seal isn't damage control. It's the alternative they picked when they refused to poison the land."

**Success consequence:**
"Logged as paradigm-changing intelligence. The QRians weren't a doomed civilization that didn't know what hit it. They were a civilization that did the math, drafted two extinction protocols, and chose the one that didn't leave the geography unlivable for whoever came next. The temple is voluntary. The seal is consent. Whatever the Source did to them, they made sure it ended *with* them, not after. The reason they refused to poison the land — that's the question we don't have an answer to yet."

**Failure consequence:**
"Without the labels, the alcove is a chemist's vault of unmarked reagents. We know they were making something. We don't know what or why they stopped. File the panel and press on."

---

## A2M5 — The Reinforced Bunker (Crew-Continuity, gated on Fuse Charges)  ✅

**Type:** R — Riddle / Cryptic Wordplay | **Cards:** 2 riddle-tablets | **Gate:** Saved Fuse Charges (Act 1) | **Evidence:** S0 (Togom's geographic theory)

**Story card brief:**
> Past the central archway, into a side gallery: one stretch of wall is *wrong*. The masonry too dense, the cuts too tight, the surface scarred where someone tried — and failed — to chisel through. Your bomb-handler runs a hand along the seam. *They sealed something behind this on purpose.* You've got the Fuse Charges. Place them. There's a name carved above the seal: **PRIEST PHYSICIST TOGOM**. He didn't want company…

**Puzzle:** Two-stage gate. (1) Place charges (saved Fuse Charges required) to breach the bunker. (2) Solve 2 cryptic riddles to release inner scroll-case.

**Tablets** (footer on each: *"Translated from QRian wordplay; the original puns are English-equivalent in form."*):

- **Togom Tablet I — ORBIT**: *"Round and round this bit, but it's not 'and.'"*
- **Togom Tablet II — GRENADE**: *"When shuffled, it's even more explosive than 'enraged.'"*

**Answers:** `orbit`, `grenade`

**Puzzle prompt:** *"Solve the seal-riddles to release the scroll-case."*

**Correct answer reveal:**
"The charges blow inward. Smoke clears. The bunker is small — barely a closet — and inside, on a stone shelf, are the scrolls. Togom's life work, sealed for whoever could solve his riddles. Your linguist unrolls the topmost. The translation reads:
> *'We discovered this place on one of our expeditions. People who came back from this place suddenly had something in their eyes — some sort of wisdom. We eventually held more expeditions to this area, and realized that all who came toward this area and stayed there for a while would suddenly become a lot more logical in their thinking. It had to do with the area itself. Nothing we destroyed or built seemed to prevent this consequence. The effect could not be destroyed.'*"

**Success consequence:**
"Logged. Togom's scrolls confirm what the Sighting Wall implied: the Source is **geographic**. Bound to this place. Empirical, observable, indestructible. The QRians proved it through repeated expedition — anyone who came near and stayed long enough caught the wisdom. They tried to destroy the effect. They couldn't. The effect *was* the place. Whatever Togom thought, he thought it carefully enough to seal it behind a wall."

**Failure consequences:**
- *Lost Fuse Charges in Act 1*: "Without the charges, the bunker stays sealed. Whatever Togom hid stays hidden. File as inaccessible and press on."
- *Riddles unsolved*: "The bunker breached, but Togom's seal-riddles refuse the wrong answers. The scrolls stay locked in their inner case, visible through the stone but out of reach. File for now and press on."

---

# JONES JUNKET (Amber)

## A2M1 — The Sealed Pantry (Artifact)  🚧

*Replaces Concept Garden brief. Briefs/consequences locked. **5 preparation-combinations TBD by Anthony**.*

**Type:** C — Conceptual Decipherment | **Cards:** 3 (painted clay shelf-labels) | **Evidence:** S5 + active-resistance (QRians medicating against the contagion)

**Story card brief:**
> Off the main chamber to the right: an archway you missed, choked with vines so thick you nearly walked past it. Push through. Inside, a storeroom — fired-clay jars on stone shelves, every one sealed, every one heavy. Some shelves still bear their painted labels. Others are bare, the labels fallen, scattered. Your historian inhales slow. *This was a pantry.* Whoever ate from this pantry was eating with intention. The other houses pocketed a few of the labels. You'll want them back to know what was being kept here…

**Puzzle:** Each shelf-cluster holds 2-3 ingredients. The combined ingredients name one preparation. Match scattered card-labels to shelves, identify 5 derived preparations. **Anthony's intent: nootropics — preparations to slow mind-drift, anchor thought against the contagion.**

**Puzzle prompt:** *"Name each preparation kept in this pantry."*

**5 preparations:** TBD by Anthony.

**Hint:** *The pantry's shelves group ingredients by preparation. Each cluster of 2-3 jars made one named compound. Match the loose labels back to their shelves. Read each cluster as a recipe. Five preparations together tell you the pantry's purpose.*

**Correct answer reveal:**
"Five preparations resolve. Read together, they aren't a meal plan. They aren't a feast list. The QRians weren't preserving food for nourishment. They were preserving food for *defense* — every jar in this room is a compound known to slow the mind's drift, to anchor thought against drift, to keep the brain from going where it didn't want to go. They were medicating themselves against the thing that was happening to them. They knew it was coming. They tried to eat their way through it."

**Success consequence:**
"Logged. The QRians weren't just observing the contagion — they were fighting it with what they had. The pantry is a record of that fight, jar by jar. Every preparation in this room is the QRians refusing, for as long as they could, to lose themselves. They lost. But they fought. That changes what we thought we knew about how this culture went under."

**Failure consequence:**
"Without the labels, the pantry is rows of sealed jars and silent shelves. We know they ate here. We don't know what they ate or why they kept it. File the storeroom as ambiguous and press on."

---

## A2M2 — The Wall of Repetitions, Amber (Environment)  ✅

*Was Lectern of Drifting Tongues. Now part of the M-trio.*

**Type:** M — Corrupted Reconstruction | **Cards:** 3 amber tiles | **Evidence:** S2 + S5 (joint)

**Story card brief:**
> Through the doorway and into the long chamber: the temple's east wall stretches floor-to-ceiling, carved with the same QRian phrase repeated over and over — hundreds of stacked rewrites, each eroded in its own way. Behind you, you realize: Drake and Croft are already at their own sections of the wall, working red tiles and purple. Your linguist drops at the amber tiles. *They wrote it this many times so the eroded ones could still be read against the rest.* Some of yours are with the others. You'll want them back…

**Jones's amber tiles** (phrase: "THIS PLACE ONCE MADE US WISE"):
- Amber 1 (held by Jones): `_HI_ PLA_E O_CE _AD_ US _IS_`
- Amber 2 (held by Drake): `T_IS _LAC_ ON_E M_DE _S W_SE`
- Amber 3 (held by Croft): `TH_S P__CE _NC_ MA_E U_ WI_E`

**Answer:** `this place once made us wise`

**Hint:** *(see Drake A2M3 — same hint applies to all three houses)*

**Correct answer reveal:**
"The wall says: **THIS PLACE ONCE MADE US WISE**. The QRians' opening confession. They didn't come here as scientists who made a discovery. They came here because being here MADE them scientists — the place itself was the source of their wisdom, and they knew it. Drake found a line below this one. Croft found another. Read all three together when the host calls it: the wall writes a sentence across every house in the room."

**Success consequence:**
"Logged. The QRians weren't naive. They came here for the wisdom this place gave them, and they knew exactly what they were trading for it. *This place made us wise* is not a celebration. It's the first line of an autopsy."

**Failure consequence:**
"Without the amber tiles, the opening line is broken. *PLACE* and *WISE* stand out from the corruption, but how they connect stays lost. File and press on."

---

## A2M3 — Vesh's Compartment (Path)  ✅

*Was Walk of the Quoting Trees. Now part of the N-trio (Three Compartments).*

**Type:** N — Conceptual Procedure | **Cards:** 6 tile-inserts | **Evidence:** S4 expansion (slave-labor, ambivalent author)

**Story card brief:**
> Off the side-passage and into a quiet alcove: a stone table set in the corner, surface marked in careful columns of inset tile-slots — some filled, some empty. Above the table, a sealed compartment, no hinge. Your linguist reads the rows. *Phrase-lock. Some tiles are with the other houses.*

**Procedure 1:** trunk → branch → leaf → ?? *(zoom-in on plant branching hierarchy)*
**Accepted:** vein

**Procedure 2:** pollen → tingle → gasp → ?? *(sneeze reflex)*
**Accepted:** sneeze, achoo, blast, spray, expel, relief

**Correct answer reveal:**
"Both passcodes enter. The compartment opens. Inside: a logged record by **Witness Chronicler Vesh**.
> *'The construction was assigned to expendable labor. The decision was made on the principle that exposure to the Source would corrupt any individual of consequence to our civilization, and we could not afford the loss. The expendable population — captured peoples, criminals, the unaccountable — were directed to the work. They built in shifts of approximately forty days. Their disposal at completion was logistical. The temple was completed in less than two cycles, an impressive output. Records to follow.'*"

**Success consequence:**
"Logged. Vesh recorded it like a ledger. No anguish, no pride — just *the construction was assigned to expendable labor.* The QRians had a class system that absorbed the moral cost without flinching. They wrote in cycles, not lives."

**Failure consequence:**
"The compartment stays sealed; the lock holds. The host's voice goes quiet — *we came close.* Vesh's record stays inside. We will not learn how cleanly he chose to write his cycles. File and press on."

---

## (Crew-Continuity) — The Hanging Garden of Names (Crew, gated on Whips)  ✅

**Type:** J — Hidden Symbol Hunt (emergent composition) | **Cards:** 3 burial-rite fragments | **Gate:** Saved Ceremonial Whips (Act 1) | **Evidence:** S2 (Priest-Scientist anatomical-grammatical fusion) | **Glyph teaching:** J + W introduced

**Story card brief:**
> Through the doors and into the dome: a greenhouse three stories high, glass long shattered, vines hanging in a forest of root-curtains. Each braid weighted at its tip with a fired-clay tag the size of a fist. Thousands. They turn slow in the draft, clicking against each other like wind chimes made of teeth. Every tag carries one painted glyph. The doors behind you grind shut. The QRians left a path through this — but only in the air…

**Puzzle:** Players receive 1 printed overhead map of the canopy showing tags within whip-reach of the central walkway. Each card names one glyph-class (sorted by phonetic articulation: closers / openers / carriers) and instructs players to "strike" each instance in numerical order. Diegetically, each strike is a precision whip-crack that drops a single tag without disturbing its neighbors. Each card's strike-sequence traces one QRian super-glyph. Three cards → three letters → **JAW**.

**Why whips required:** The canopy hangs at lethal density — any contact-retrieval (poles, ropes, hands) tangles the curtain and drops it onto the floor pressure-plate that completes the seal. Only a precision whip-crack can drop a single tag without disturbing its neighbors.

**Cards:**
- **Card 1 — Burial-Rite Fragment, Folded** *(traces J — NEW)*: "Strike first the speakers — every glyph that closes the mouth. The dead spoke our names back to us in the order we forgot them; we wrote each name on the bone that had carried it. Crack each in turn."
- **Card 2 — Burial-Rite Fragment, Charred** *(traces A — known)*: "Then the openers — glyphs that part the lips. We did not bury our scholars. To bury would have meant we stopped recording. We hung them instead, where the wind could still move through what they had said. Crack in turn; let each one fall."
- **Card 3 — Burial-Rite Fragment, Half-Erased** *(traces W — NEW)*: "Last, the carriers — glyphs that ride the breath out. A name is only the shape the jaw makes when the body is gone. Strike them in the order the breath leaves."

**Puzzle prompt:** *"What hangs in this garden?"*

**Answer:** `jaw` (alternatives: `JAW`, `Jaw`, `jaws`, `bone`)

**Hint:** *The QRians sorted their dead by how each glyph used the mouth — closers, openers, carriers. Each card defines one class. Mark every instance of that class on the canopy map; the order you crack them in traces a letter.*

**Correct answer reveal:**
"Three shapes resolve: J — A — W. The tags weren't tags. When a QRian died, they kept the speaking-bone — the only part of the body that had carried words. Bone crumbled; clay replaced it; the names stayed. This garden isn't a wonder of the temple. It's a library of voices, kept in the shape of the part of the body that voiced them. You weren't reading a wall. You were reading the dead. They classified their own corpses by phonetic articulation. They could not stop classifying. They could not stop."

**Success consequence:**
"The Priest-Scientists didn't separate biology from grammar — or grammar from astronomy, or astronomy from law, or law from medicine. Everything was the same act: classify, record, preserve. The jaw was an organ, a tool, a punctuation mark, and a sacred relic, all at once and without contradiction. They didn't bury their dead because burial would have meant *stopping looking*. Logged. Whatever the Source did to them, it took a culture that had already turned everything, including itself, into text."

**Failure consequence:**
"Without the whips, the canopy is unreachable — the only path through this room is under it. The team picks along the central walkway with eyes down, hands clenched, the dead clicking softly above. We will not learn what was hung here. File and press on."

---

## A2M5 — The Ceiling Inscription (Utility, ungated)  ✅

**Type:** R — Riddle / Cryptic Wordplay | **Cards:** 2 riddle-tablets | **Gate:** None (deliberately ungated — Sefa's scrolls are guaranteed-recoverable for game-safety) | **Evidence:** S1 + S0 (Sefa's linguistic theory of the Source)

**Story card brief:**
> Northeast corner of the chamber, where the ceiling slopes low to meet the wall: faint chiseled glyphs catch the lamp-light. Your linguist freezes. *That's a name.* The carvings name an author the others walked past — and below the inscription, a stone tile sits slightly proud of the wall. A pull-tab. The compartment behind it is untouched. The QRians sealed this one with patience, expecting only readers. Above the tile: **BOTANIST THEOLOGIAN SEFA**. She wanted to be found…

**Tablets** (footer on each: *"Translated from QRian wordplay; the original puns are English-equivalent in form."*):
- **Sefa Tablet I — FLOWER**: *"It is said to flow, but it doesn't. Instead, it stands beautifully."*
- **Sefa Tablet II — BUTTERFLY**: *"You would think that you put on your bread, but instead it soars like a queen."*

**Answers:** `flower`, `butterfly`

**Puzzle prompt:** *"Solve the seal-riddles to release the scroll-case."*

**Hint:** *Two different cryptic techniques. The first sounds like something else when said out loud. The second uses "queen" as a clue to a specific kind of insect.*

**Correct answer reveal:**
"The tile pulls away. Behind it, exactly the depth of an arm: a tightly-wrapped scroll-bundle. Sefa's writing, untouched. Your linguist unrolls the topmost. The translation reads:
> *'All those affected by the Source suddenly carried the power of a blocky, descriptive language. The script exploded throughout our civilization — everyone began using these blocky letters to communicate. It seemed effective at first. It was a powerful way to evolve our language. But it affected our entire culture, intimately, perhaps too intimately.'*"

**Success consequence:**
"Logged. Sefa's scrolls reveal the **mechanism**: the Source manifested in language. The QRians' blocky, descriptive script — every glyph the team has been decoding all year — is the Source's signature. It exploded through the population, evolved their language, and through that language, evolved the people. The script and the obsession arrived together. To write QRian was to be touched. She left the door open. She wanted you to read this."

**Failure consequence:**
"Sefa's compartment is open but the inner cylinder won't release without the riddle-answers. Her scrolls stay wrapped in their case. We came so close. File for now and press on."

---

# CROFT COMPANY (Purple)

## A2M1 — The Reckoning Floor (Environment)  ✅

**Type:** J — Hidden Symbol Hunt (emergent composition) | **Cards:** 4 calculation tablets | **Evidence:** S3 (The Contagion) + S2 (math/language fusion)

**Story card brief:**
> Down the steps and across the chamber floor: a wide stone slab, set with thousands of pebbles, each carved with a single number. Something was being calculated. Your engineer crouches and starts mapping it — and stops. In QRian, math and language sit close enough that a calculation doesn't end in a number; it ends in a word. The other houses have pocketed the calculator's working notes. You'll want them back to read what's frozen here…

**Puzzle:** Printed top-down map of the pebble floor — dense grid of Arabic numerals. Some instances of digits 3, 5, 7, 9 bear small calculation-sequence numbers. Each card identifies one digit-class and its role in the calculation. Connect numbered instances in order to trace one super-glyph each. Four super-glyphs spell **TIME**.

**Digit-to-letter mapping:**
- Card 1 — Digit **3** — role: the unit being measured → traces **T**
- Card 2 — Digit **5** — role: the rate of consumption → traces **I**
- Card 3 — Digit **7** — role: the elapsed measure → traces **M**
- Card 4 — Digit **9** — role: the projected remainder → traces **E**

**Cards** (distributed Croft 1 / Drake 1 / Jones 2):
- **Card 1 — Three Calculation Tablet**: "Three-class pebbles mark the unit. Find every 3 on the floor record bearing a sequence number. Trace them in order. Their path is the first letter of the answer. (Math here writes itself in language.)"
- **Card 2 — Five Calculation Tablet**: "Five-class pebbles mark the rate. Find every numbered 5. Trace in sequence. The shape is the second letter."
- **Card 3 — Seven Calculation Tablet**: "Seven-class pebbles mark the elapsed. Trace every numbered 7 in sequence."
- **Card 4 — Nine Calculation Tablet**: "Nine-class pebbles mark the remainder. Trace every numbered 9 in sequence."

**Puzzle prompt:** *"What were the QRians measuring?"*

**Answer:** `time` (alternatives: `TIME`, `Time`)

**Correct answer reveal:**
"Four shapes resolve: T — I — M — E. Not a quantity. Not a duration. The word itself. The QRians' math couldn't separate counting from spelling — when they ran a calculation about something, the calculation produced the name of what they were counting. The answer to *how long do we have* was TIME, written out in the only thing that could write it: a measurement of itself. The calculator embedded the question and the answer in the same field of stones, and the floor froze where the answer ran out. They knew. They counted. They knew."

**Success consequence:**
"The contagion didn't take them by surprise. They had a model of it. They had a measurement of it. They had a calculation that ran in the same fused math-language that was eating them alive — which is why the answer it produced was a word, and why the word was the same as the question. Logged as decisive evidence. They saw it. They ran out. Move."

**Failure consequence:**
"Without the calculator's working notes, the pebble floor is a frozen abacus running no software. We know they were measuring something. Not what. File the field as ambiguous and press on."

---

## A2M2 — The Sighting Wall (Artifact)  🚧

*Was Heliotrope Turn. Briefs/consequences locked. **5 lens-observation combinations TBD by Anthony**.*

**Type:** C — Conceptual Decipherment | **Cards:** 3 (lens-labels) | **Evidence:** S0 *(first first-coverage of S0)*

**Story card brief:**
> Up the ledge your climber uncovered: the chamber narrows to a wall, and the wall is full of slits. Thin vertical cuts, each one no wider than a hand, set at varying heights, looking out — out, over the canopy, the basin, the far ridges. Set into many of the slits: amber lenses, polished smooth, each one labeled in a careful hand. Other slits are bare; their labels gone, scattered to the chamber below. Your climber goes still. *This wasn't a fortress wall.* It was an eye…

**Puzzle:** Each cluster of 2-3 slits/lenses tracked one observable phenomenon. Match scattered card-labels to lenses, identify 5 derived observations. **Anthony's intent: weather + astronomical patterns.**

**Puzzle prompt:** *"Name each phenomenon the QRians were observing."*

**5 observations:** TBD by Anthony.

**Hint:** *Each cluster of 2-3 slits aimed at one phenomenon. Match the loose labels to their lenses. Read each cluster as a single observation. Five observations together tell you what the QRians were trying to find.*

**Correct answer reveal:**
"Five observations resolve. Not weather forecasting. Not crop timing. Not warfare reconnaissance. The QRians were *triangulating their own location in the universe*. They watched the sky for what they couldn't otherwise know — where they sat in the bigger pattern, what their place was relative to fixed stars, how the weather over this valley differed from the weather over every other valley. This place was special to them. They built this wall to find out *how* special. They were measuring whether the Source was *here* — bound to this geography — or whether it was bigger than that."

**Success consequence:**
"Logged. The QRians weren't a culture that took its location for granted. They were *cosmographers*, measuring where they sat with the patience of people who suspected the answer mattered. The Source was tied to this place; the wall is them confirming that. That's why they built the temple here. That's why the seal seals here. The geography is the disease, and they were the only people who knew it."

**Failure consequence:**
"Without the labels, the lenses look out on an indifferent horizon. We know they watched. We don't know what they were watching for. File the wall as observatory of unknown purpose and press on."

---

## A2M3 — Krane's Compartment (Path)  ✅

*Was Wall That Grows. Now part of the N-trio (Three Compartments).*

**Type:** N — Conceptual Procedure | **Cards:** 6 tile-inserts | **Evidence:** S4 expansion (slave-labor, "liked-it" author)

**Story card brief:**
> Off the side-passage and into a tight alcove: a stone table set in the corner, mechanism still snug — rows of inset tile-slots, some filled, some empty. Above the table, a sealed compartment, no hinge. Your point climber checks the slots. *Mechanism's plain. The other houses have what we need.*

**Procedure 1:** clear → gather → darken → ?? *(approaching storm)*
**Accepted:** storm, rain, thunder, lightning, pour, downpour, break

**Procedure 2:** ebb → gather → wave → ?? *(water motion cycle)*
**Accepted:** crash, flow, break, crest, recede, surge, foam, wash

**Correct answer reveal:**
"Both passcodes enter. The compartment opens. Inside: a working note by **Foreman Geometer Krane**.
> *'The labor system was elegant. We needed builders we could afford to lose. We had them. The captives were efficient when motivated, the criminals when threatened, and the unmarked when convinced of purpose. We assigned tasks by aptitude — the strong to lifting, the deft to inlay, the small to ducting. The work proceeded smoothly. Their elimination at completion was a kindness; we would not return them to a world they could not rejoin. The temple stands as proof.'*"

**Success consequence:**
"Logged. Krane *liked* it. Liked the elegance. Liked the system. The QRians weren't a uniform culture in their last days — some of them found the slave-system *beautiful*. The temple stands because somebody ran the math and called the math good."

**Failure consequence:**
"The compartment stays sealed; the lock holds. The host's voice goes quiet — *we came close.* Krane's record stays inside. We will not learn what he found elegant, or why. File and press on."

---

## A2M4 — The Wall of Repetitions, Purple (Utility)  ✅

*NEW Croft mission filling the M-type slot. Part of the M-trio.*

**Type:** M — Corrupted Reconstruction | **Cards:** 3 purple tiles | **Evidence:** S2 + S5 (joint)

**Story card brief:**
> Through the doorway and into the long chamber: the temple's east wall stretches floor-to-ceiling, carved with the same QRian phrase repeated over and over — hundreds of stacked rewrites, each eroded in its own way. Behind you, you realize: Drake and Jones are already at their own sections of the wall, working red tiles and amber. Your point climber kneels at the purple tiles. *They wrote it this many times so the eroded ones could still be read against the rest.* Some of yours are with the others. You'll want them back…

**Croft's purple tiles** (phrase: "UNTIL WE SLOWLY REALIZED"):
- Purple 1 (held by Croft): `_NT_L W_ SL_WL_ R_ALI_ED`
- Purple 2 (held by Drake): `U_TI_ WE _LO_LY _EA_IZ_D`
- Purple 3 (held by Jones): `UN_IL _E S_OW_Y R_AL_ZE_`

**Answer:** `until we slowly realized`

**Correct answer reveal:**
"The wall says: **UNTIL WE SLOWLY REALIZED**. The middle of a sentence. The hinge. Wisdom turning to obsession, slowly enough to inscribe the realization mid-fall. Jones found the start. Drake found the end. Read all three together when the host calls it: there's a complete confession on this wall."

**Success consequence:**
"Logged. They watched themselves become what they were becoming, and they had a word for that watching: *slowly.* They had time to write it down. They had time to anticipate a reader. We are the reader."

**Failure consequence:**
"Without the purple tiles, the middle line stays in pieces. *SLOWLY* survived the most readings. Whatever was happening slowly, we don't know what. File and press on."

---

## A2M5 — The High Ledge (Crew-Continuity, gated on Grappling Rigs)  ✅

**Type:** R — Riddle / Cryptic Wordplay | **Cards:** 2 riddle-tablets | **Gate:** Saved Grappling Rigs (Act 1) | **Evidence:** S0 (Yenus's metaphysical theory)

**Story card brief:**
> Up the gallery wall: a small ledge, twelve feet of sheer rock above the chamber floor — too high to jump, too smooth to climb. A compartment is cut into the face of it, sealed with a wooden lid that looks impossibly fresh. Your point climber tilts her head back. *Whoever lived up there wanted to be alone.* You've got the Grappling Rigs. Set the lines. A name is carved into the lid above: **PHILOSOPHER MATHEMATICIAN YENUS**. He kept his work where the floor couldn't reach…

**Tablets** (footer on each: *"Translated from QRian wordplay; the original puns are English-equivalent in form."*):
- **Yenus Tablet I — PRESENT**: *"The gift you always have, at all times."*
- **Yenus Tablet II — GROUND**: *"Add a G; it describes the Earth. Remove the G; it describes the Earth."*

**Answers:** `present`, `ground` (alternative for II: `round` if entered as the "remove the G" form)

**Puzzle prompt:** *"Solve the seal-riddles to release the scroll-case."*

**Hint:** *Two different cryptic techniques. The first riddle has two meanings at once. The second tells you what to do with a letter.*

**Correct answer reveal:**
"The lid lifts. The compartment is small but dry — Yenus chose his ledge well. Inside: the scrolls. Your engineer unrolls the topmost. The translation reads:
> *'This place was special. Almost divine, even. It gave us a sense of understanding which vastly accelerated our civilization. We understood mathematics, logic, physics — somehow suddenly, somehow intuitively. And somehow obsessively. We used our new knowledge to build our civilization to great heights. Farmers became mathematicians; merchants became astronomers; we became the envy of the land. Little did we know that this obsession was an unstoppable force…'*"

**Success consequence:**
"Logged. Yenus's scrolls add the *meaning* to Togom's measurements: the Source didn't just affect them, it *gave* them — mathematics, logic, physics, all suddenly, all intuitively. Civilization-altering wisdom for free. They built greatness on it. They didn't realize until too late that the wisdom and the obsession were the same thing. The unstoppable force. He kept his work above the floor because the floor was where the others were going wrong."

**Failure consequences:**
- *Lost Grappling Rigs in Act 1*: "Without the rigs, the ledge is unreachable. Yenus chose his elevation well. File as inaccessible and press on."
- *Riddles unsolved*: "The lid opens but the inner case won't release. Yenus's riddles refuse the wrong answers. The scrolls are inches away and behind a thousand years of patience. File and press on."

---

# Cross-cutting Notes

## R-trio synthesis (The Three Theorists)

Together the three Priest-Scientists deliver a complete origin theory of the Source:
- **Togom = WHERE** (geographic radiation field, indestructible, place-bound)
- **Yenus = EFFECT** (cognitive uplift = obsession; civilization-altering; the wisdom and the disease are the same)
- **Sefa = HOW** (through the blocky QRian script — the script we have been decoding all year is the Source's transmission medium)

To write QRian was to be touched. To read it... is what the team has been doing all along. Year-long glyph-decoding gameplay payoff.

**Recoverability**: Sefa's scrolls are guaranteed-recoverable (Jones is ungated). So the linguistic mechanism is always delivered. Togom's geographic and Yenus's metaphysical depend on Act 1 crew-item preservation.

## M-trio joint reveal (Wall of Repetitions)

When all three houses solve, the host reads the complete sentence aloud:
> *THIS PLACE ONCE MADE US WISE*
> *UNTIL WE SLOWLY REALIZED*
> *IT WAS MAKING US OBSESSED*

This is the QRians' confession at scale, written hundreds of times into the same wall so corruption could be averaged out across copies. They diagnosed themselves in the act of being broken.

## N-trio synthesis (The Three Compartments)

Three Priest-Scientists wrote different tones about the temple's slave-labor construction:
- **Drevu** (terrified, asks forgiveness)
- **Vesh** (ambivalent, ledger-clinical)
- **Krane** (liked it, found the system "elegant")

The QRians weren't morally uniform in their final days. The temple was built by enslaved labor — captives, criminals, "the unaccountable" — eliminated upon completion because the Source would have prevented their reintegration. Three distinct authorial tones. Croft uncovers the worst author, which is the cruelest beat: the engineer-pragmatist house finds their philosophical kin is the monstrous one.

---

# Pending design tasks

## C-trio concepts (5 each, by Anthony)

| Mission | Atom type | 5 derived concepts must be… | Reveal theme |
|---|---|---|---|
| **Jones — Sealed Pantry** | Ingredients on shelves | 5 named preparations | Nootropics — anti-contagion medication |
| **Croft — Sighting Wall** | Lens-types in slits | 5 named phenomena | Cosmographic triangulation (S0) |
| **Drake — Reagent Alcove** | Reagents at stations | 5 named compounds | Area-weapons abandoned for the temple (S4) |

Each concept = 2-3 atoms combined → 1 named result. Once Anthony drafts these (15 concepts total + atom palettes), the C-trio is fully designed and Act 2 is complete.

## Grid generation

Three printed-artifact grids still need to be generated (QRian font is finalized — unblocked):
- **Powder of the Quiet Bed** (J): ~20×15 QRian-glyph grid; super-glyphs B, Y, E
- **The Reckoning Floor** (J): ~20×15 Arabic-numeral grid; digits 3/5/7/9 trace T, I, M, E
- **The Hanging Garden of Names** (J): ~20×15 jawbone-canopy QRian-glyph grid; super-glyphs J, A, W

Full generation specs for each are saved in memory at `~/.claude/projects/-Users-aaddo-src-cardsight/memory/project_act2_progress.md`.
