...
│   │   │   │   ├── games.ts         # Game CRUD, duplicate, history timeline arm/reset, finale selection
...
│   │   │   │   └── RequiredItems.tsx     # Required item badges for missions
...
│   │   │       ├── HostConsole.tsx        # Mobile host console (6-tab: pulse, activity, cards, missions, showtime, finale)
...
│   │   ├── services/
│   │   │   ├── card.service.ts       # Player scan flow (lockout, self-destruct, answers, mission auto-complete, history timeline checks)
...
│   ├── finale.ts              # Finale ruleset, clause compatibility, scoring, consequence summaries
│   └── physical-cards.json    # 54 physical card definitions (UUIDs, colors, names, Short.io links)
├── story/
│   ├── README.md
│   └── act3-history-notes.md  # Act 3 history timeline design notes and card-writing template
...
- **Mission** — a task for specific house(s) in a specific act. ~6 per house per act, teams complete 3-4. Has title, description, `puzzleDescription` (shown to players when they scan the mission QR), `storySheetBlurb` (markdown printed under the story brief, alongside the QR code), required clue sets (references CardSet IDs with counts), optional mission card link, optional Design for theming, and polymorphic answer template. Player-scannable via QR codes at `/m/:missionId`. Many-to-many with houses via MissionHouse (supports collaborative cross-house missions). Contains consequence texts (completed/not completed) with optional images. Can be locked with `lockedOut`/`lockedOutReason` (used by future act consequences system). Reorderable via drag-and-drop in the admin (persists `sortOrder`); story sheet print uses this order.
- **MissionHouse** — join table linking missions to houses. A mission can belong to one or multiple houses.
- **StorySheet** — narrative document per house per act. Contains the situation description (markdown). Has title, content, admin notes. Unique constraint on game+house+act. Editable in admin with house tabs and act grouping. Print view renders content as plain markdown, then auto-renders the house's missions for that act as banded blocks (title, `storySheetBlurb` markdown, QR code) ordered by `Mission.sortOrder`. Duplicated with games.
...
- **History cards are a first-class card subtype** — they reuse the normal card scan path and physical/NFC URLs, but intentionally bypass the normal examine/self-destruct/answer flow. History cards still show the physical-card flash, then go straight to their text. They never show the blur nudge or the "What if you'd written it down?" overlay.
- **Reference cards are a second direct-view subtype** — they are for public read-only materials like clause cards or other explanatory cards. They use the same QR/NFC flow, skip the examine gate, skip self-destruct and answer UI, and always open directly to their text after the physical-card flash. Unlike history cards, they never participate in timeline verification.
- **History timeline verification is host-armed** — the host console's Pulse tab exposes `Arm Timeline Check` and `Reset Timeline Check`, along with solved/armed/progress state. When armed, scans of history cards are interpreted as a timeline attempt against the configured `historyTimelineOrder` values for that game. A wrong card fails immediately, disarms the timeline, and resets progress to 0. The host must re-arm before another attempt. A fully correct sequence sets `historyTimelineSolvedAt`, disarms the timeline, and the player sees a confetti-backed `HISTORY VERIFIED` success screen.
- **Finale adjudication is host-entered** — the host console's Finale tab is the source of truth for the endgame package. The host selects a single major outcome and checks the negotiated clause cards manually based on what the room agreed to.
- **Finale scoring is shared and deterministic** — `shared/finale.ts` defines the three outcomes, all clause cards, compatibility rules, the Jones/Croft/Drake house weights, and the `Vindicated / Compromised / Horrified` thresholds. The same evaluator is used to persist the selected package and to render the host preview, so the numbers and consequences stay in sync.
...
- **Splash gate** — every card shows a splash screen on first scan displaying the item name prominently (large, centered, with a small "Item" label above it). Staged reveal: warning text ("Only examine if you're sure you want to use this item for your mission") fades in after 1.2s, then the EXAMINE button fades in after 4s. The warning lands before the button gives players something to press, reinforcing that examining is a commitment. Once examined (`examinedAt` is set on Card), subsequent scans skip the splash and go straight to content. Timer starts on examine, not on scan.
...
- **Print rendering: themed-for-screen, not always paper-safe.** The themed print pages (`StorySheetPrint`, `ConsequencePrint`, `Act3CardsPrint`) are designed for screen drama — dark backgrounds, layered radial-gradients, decorative `mask-image` effects, and `print-color-adjust: exact`. On cheap printers and most PDF generators this fails in two known ways: (1) layered low-opacity gradients render as muddy near-black bands instead of the subtle effects they look like on screen, and (2) CSS `mask-image` is silently dropped, gating any decorative layer that uses it (notably the Act 3 era backgrounds), so paper output is "dark base color with no detail." Each print page therefore ships with a **"Plain print" Switch** in its toolbar that forces a white card background, dark text, and hides decorative layers — toggle on for paper, off for the dramatic screen preview. If you add a new themed print page, follow the same pattern; don't assume `print-color-adjust: exact` will Just Work on real hardware.
...
- **Visibility guard** — blurs content when player switches away from the browser tab (anti-screenshot). Has an optional "blur nudge" mode (toggled per-game via `blurNudgeEnabled` on Game, controllable from the host console's Pulse tab). When enabled, returning from a tab-away shows a diegetic message ("You put the item down and now you're scrambling to find where you placed it... What if you'd just written down the key details?") for 5 seconds before unblurring. Trains players to write things down without punitive re-scan requirements. History cards skip this overlay entirely.
- **History timeline authoring** — history cards are authored in the normal card editor by setting `Subtype = History` and assigning a unique `Timeline Order`. This keeps the verification system data-driven and tied to the same QR/NFC card records already used in play.
...
```
# House cookie (analytics tagging — does not affect routing or content)
POST  /api/house-claim/:slug             # Sets cs_house cookie for the active game's house with this slug

# Cards
GET   /api/cards/:cardId          # Card content (respects lockout, self-destruct, includes item name)
...
**Built:**
- Full scan flow (QR → physical card flash → splash gate (item name + delayed EXAMINE) → content → self-destruct → answer → feedback). Simple cards show item directly; complex cards show puzzle with answer input, then reveal item content on solve.
...
- `clueVisibleCategory` shown to players as the item name on card content (so they know what item they're collecting for their inventory)
