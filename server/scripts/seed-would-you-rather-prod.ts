/**
 * seed-would-you-rather-prod.ts — manual production reseed gate
 *
 * Loads server/.env.production, confirms with the operator, then runs
 * the Would You Rather seed against the production database.
 *
 * Run: pnpm --filter server db:seed-would-you-rather:prod
 */
...
const SEED_SCRIPT = resolve(import.meta.dirname, "..", "prisma", "seed-would-you-rather.ts");
...
async function main() {
  ...
  console.log("  PRODUCTION RESEED — Would You Rather");
  console.log("─".repeat(60));
  console.log(`  Target:  ${redactUrl(dbUrl)}`);
  console.log("");
  console.log("  This will DELETE any existing production copy of");
  console.log("  Would You Rather before recreating it from the");
  console.log("  current seed script.");
  console.log("");
