...
export function MissionAnswerInput({ missionId, houseId, answerMeta, onCompleted }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await postMissionAnswer(
        missionId,
        value.trim(),
        houseId,
        getSessionHash(),
  ...
  return (
    <div style={{ marginTop: "2rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "8px",
              border: "none",
              background: lastResult === "correct" ? "#69f0ae" : "var(--card-accent-color)",
              color: "#000",
      ...
      {showHintTeaser && (
        <div style={{ marginTop: "0.5rem", fontSize: "0.8rem", opacity: 0.4, fontStyle: "italic" }}>
